...
Мы оперируем только ссылками на структуры дерева -
сама функция выделяет константную память O(1) для хранения ссылок на узлы которые требуется удалить и его родителя
из за того что мы рекурсивно вызываем ее проходя по дереву -
то на хранение стека будем тратить O(H) - где H - высота дерева
